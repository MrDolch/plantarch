...
import playground.erm.jpa.Somebody
...
class ClassDiagramTest {
  ...
  fun testClassPackageDiagram() {
    ...
    val plantuml = testee.toPlantuml()
    println(plantuml)
    Paths.get("target", "Test_ClassPackageDiagram.plantuml")
      .toFile().writer().use {
        it.write(plantuml)
      }
  }
  ...
  @Test
  fun testClassDependencyDiagram() {
    val testee = ClassDiagram(
      "Test Class Diagram of Car",
      "Shows the dependencies of the example package jpa"
    )
    ...
    testee.analyzeClass(Somebody::class.java)
    ...
    val plantuml = testee.toPlantuml()
    println(plantuml)
    Paths.get("target", "Test_ClassDependencyDiagram.plantuml")
      .toFile().writer().use {
        it.write(plantuml)
      }
  }
